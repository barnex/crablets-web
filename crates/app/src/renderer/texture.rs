...
pub fn load_embedded_atlas(device: &wgpu::Device, queue: &wgpu::Queue) -> Result<Texture> {
	...
	let img = image::load_from_memory(include_bytes!("../../../../assets/textures/atlas.png"))?.flipv(); // 👈 flip because images are stored with Y down, but we use Y up.
...
impl Texture {
	...
	fn make_bind_group(device: &wgpu::Device, view: &wgpu::TextureView, sampler: &wgpu::Sampler) -> wgpu::BindGroup {
