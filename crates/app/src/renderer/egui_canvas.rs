...
impl EguiCanvas {
	...
	pub fn paint(&mut self, ui: &mut egui::Ui, rect: Rect, scenegraph: Scenegraph) -> egui::layers::ShapeIdx {
		ui.painter().add(egui_wgpu::Callback::new_paint_callback(rect, CanvasEguiCallback { scenegraph }))
	}
}
...
impl CallbackTrait for CanvasEguiCallback {
	fn prepare(&self, _device: &wgpu::Device, _queue: &wgpu::Queue,
        _screen_descriptor: &egui_wgpu::ScreenDescriptor,
		 egui_encoder: &mut wgpu::CommandEncoder, callback_resources: &mut egui_wgpu::CallbackResources) -> Vec<wgpu::CommandBuffer> {
		if let Some(renderer) = callback_resources.get_mut::<Renderer>() {
			renderer.prepare(egui_encoder, &self.scenegraph)
		} else {
