...
#[derive(Deserialize, Clone, Serialize, Reflect)]
#[serde(default)]
...
impl Default for DebugOpts {
	fn default() -> Self {
		Self {
			...
			congestion_overlay: false,
			reservation_overlay: true,
			taskman_empty_hands: true,
			taskman_give_work: true,
			expire_factory_reservations: false, // << TODO: true
			tick_factories: true,
			god_mode: false,
		}
	}
